...
function findBackward(
  text: string,
  index: number,
  options: { disableOpenCloseBrack?: boolean } = {}
): SearchResult {
  ...
  for (let i = index; i >= 0; i--) {
    ...
    if (!options.disableOpenCloseBrack) {
...
export function selectText(options: {
  includeBrack: boolean;
  disableOpenCloseBrack?: boolean;
}) {
  ...
  const findOptions = {
    disableOpenCloseBrack: options.disableOpenCloseBrack
  };
  var backwardResult = findBackward(
    searchContext.text,
    searchContext.backwardStarter,
    findOptions
  );
  var forwardResult = findForward(
    searchContext.text,
    searchContext.forwardStarter,
    findOptions
  );
  ...
  while (
    ...
    forwardResult = findForward(
      searchContext.text,
      forwardResult.offset + 1,
      findOptions
    );
  }
  while (
